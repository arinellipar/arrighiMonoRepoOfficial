        ...
        public async Task SeedPermissoesGruposAsync()
        {
            ...
            foreach (var grupo in grupos)
            {
                switch (grupo.Nome)
                {
                    ...
                    case "Faturamento":
                        // Acesso total exceto módulo Usuario (similar ao administrador)
                        var permissoesFaturamento = permissoes.Where(p => p.Modulo != "Usuario").ToList();
                    ...
                    case "Cobrança e Financeiro":
                        // Apenas visualização de tudo, exceto módulo Usuario
                        var permissoesCobrancaFinanceiro = permissoes.Where(p =>
                            p.Acao == "Visualizar" && p.Modulo != "Usuario").ToList();
                    ...
                    case "Gestor de Filial":
                        // Acesso total aos dados da filial, exceto módulo Usuario
                        var permissoesGestorFilial = permissoes.Where(p => p.Modulo != "Usuario").ToList();
