...
interface ClienteAuthContextType {
  ...
  isLoading: boolean;
  error: string | null;
  login: (documento: string, senha: string) => Promise<boolean>;
  logout: () => void;
  clearError: () => void;
}
...
export function ClienteAuthProvider({ children }: { children: ReactNode }) {
  ...
  // Login do cliente por CPF ou CNPJ com senha
  const login = useCallback(async (documento: string, senha: string): Promise<boolean> => {
    setIsLoading(true);
    setError(null);
    ...
    if (!tipoDocumento) {
      setError("Documento inválido. Digite um CPF (11 dígitos) ou CNPJ (14 dígitos).");
      setIsLoading(false);
      return false;
    }
