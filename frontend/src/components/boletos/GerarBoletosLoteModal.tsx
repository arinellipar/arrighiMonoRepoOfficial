...
export function GerarBoletosLoteModal({
  ...
  const fetchPreview = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5101/api"}/Boleto/gerar-lote/preview`,
        {
          headers: {
  ...
  const executarGeracao = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5101/api"}/Boleto/gerar-lote`,
        {
          method: "POST",
          headers: {
  ...
  const getStatusBadge = (status: string) => {
    const statusConfig: Record<string, { bg: string; text: string; icon: any }> = {
      SUCESSO: { bg: "bg-green-500/20", text: "text-green-400", icon: CheckCircle },
      PARCIAL: { bg: "bg-amber-500/20", text: "text-amber-400", icon: AlertTriangle },
      ERRO: { bg: "bg-red-500/20", text: "text-red-400", icon: XCircle },
      NENHUM: { bg: "bg-neutral-500/20", text: "text-neutral-400", icon: Clock },
    };
    ...
    return (
      <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full ${config.bg} ${config.text} font-medium`}>
          ...
          <div className="flex-1 overflow-y-auto p-6">
            ...
            {loading && (
              <div className="flex flex-col items-center justify-center py-12">
                ...
                <p className="text-neutral-300 text-lg">Carregando preview...</p>
              </div>
            )}
                          ...
                          <div className="max-h-[300px] overflow-y-auto">
                            <table className="w-full">
                              ...
                              <tbody className="divide-y divide-neutral-800">
                                {preview.contratos.map((contrato: ContratoPreview) => (
                                  <tr
                      ...
                      <table className="w-full">
                        ...
                        <tbody className="divide-y divide-neutral-800">
                          {resultado.boletosGerados.map((boleto) => (
                            <tr key={boleto.boletoId} className="hover:bg-neutral-800/30">
